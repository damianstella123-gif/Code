...
export default function TabBudget({ event, suppliers }: { event: Event; suppliers: Supplier[] }) {
  ...
  // Eliminazione di una voce di costo: agisce sulla tabella di dettaglio
  // corrispondente alla categoria della riga.
  async function deleteBudgetLine(item: BudgetLine) {
    if (!isSupportedTable(item.table)) {
      showToast('Questa voce non puo essere eliminata da qui.', 'error')
      return
    }
    ...
    if (error) {
      showToast(`Impossibile eliminare la voce: ${error.message}`, 'error')
      return
    }
    if (count === 0) {
      showToast('Voce non eliminata: permessi insufficienti su questo evento.', 'error')
      return
    }
    setDeletingLine(null)
    showToast('Voce eliminata dal budget.', 'success')
    await loadData()
  }
          ...
          <div className="px-4 pb-4 pt-2" style={{ background: 'var(--bg)' }}>
            ...
            {item.stato_conferma === 'richiesto' && (
              <div className="flex items-center justify-between px-3 py-2 rounded-lg text-xs" style={{ background: 'rgba(255,194,75,0.08)', border: '1px solid rgba(255,194,75,0.2)' }}>
                ...
                {isSupportedTable(item.table) && (
                  <button
                    onClick={() => setDeletingLine(item)}
                    className="flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-medium transition-colors ml-2"
                    style={{ background: 'transparent', color: 'var(--red2)', border: '1px solid var(--red2)' }}
                    title="Elimina questa voce dal budget"
                  >
      ...
      {deletingLine && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <div onClick={e => e.stopPropagation()}
            style={{ background: 'var(--panel-solid)', border: '1px solid var(--line)', borderRadius: 14, padding: 22, maxWidth: 420, width: '100%' }}>
            ...
            <p style={{ fontSize: 13, color: 'var(--muted)', lineHeight: 1.55, marginBottom: 6 }}>
              {deletingLine.descrizione || 'Voce senza descrizione'}
            </p>
            <p style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 18 }}>
              {deletingLine.categoria}
              {deletingLine.fornitore ? ` · ${deletingLine.fornitore}` : ''}
            </p>
            <div className="flex justify-end gap-2">
              <button onClick={() => setDeletingLine(null)}
                className="px-4 py-2 rounded-lg text-sm"
                style={{ background: 'var(--panel2)', color: 'var(--text)', border: '1px solid var(--line)' }}>
                Annulla
              </button>
              <button onClick={() => deleteBudgetLine(deletingLine)}
                className="px-4 py-2 rounded-lg text-sm font-medium"
                style={{ background: 'var(--red2)', color: '#fff', border: 'none' }}>
                Elimina voce
              </button>
            </div>
          </div>
