...
type FormModifica = {
  ...
  company: string; job_title: string; dietary_requirements: string
  accessibility_requirements: string; custom_answers: Record<string, unknown>
}
...
export default function RegistrationParticipantsManager({ eventId, siteId, readOnly }: Props) {
  ...
  function apriModifica(reg: EventRegistration) {
    setInModifica(reg.id)
    setFormModifica({
      ...
      accessibility_requirements: reg.accessibility_requirements ?? '',
      custom_answers: (reg.custom_answers ?? {}) as Record<string, unknown>,
    })
  }

  async function salvaModifica(id: string) {
    if (!formModifica) return
    setSalvataggio(true)
    try {
      const { data, error } = await supabase.rpc('staff_update_registration', {
        p_registration_id: id,
        p_first_name: formModifica.first_name,
        p_last_name: formModifica.last_name,
        p_email: formModifica.email,
        p_phone: formModifica.phone,
        p_company: formModifica.company,
        p_job_title: formModifica.job_title,
        p_dietary: formModifica.dietary_requirements,
        p_accessibility: formModifica.accessibility_requirements,
        p_custom_answers: formModifica.custom_answers,
      })
      if (error) { showToast(error.message, 'error'); return }
      const r = data as { error?: string; nessuna_modifica?: boolean }
      if (r?.error) {
        const messaggi: Record<string, string> = {
          EMAIL_NON_VALIDA: 'Indirizzo email non valido.',
          EMAIL_GIA_USATA: 'Questo indirizzo email e gia usato da un altro iscritto.',
          UNAUTHORIZED: 'Non hai i permessi per modificare questa registrazione.',
          REGISTRAZIONE_NON_TROVATA: 'Registrazione non trovata.',
        }
        showToast(messaggi[r.error] ?? r.error, 'error')
        return
      }
      showToast(r?.nessuna_modifica ? 'Nessuna modifica da salvare.' : 'Dati aggiornati.', 'success')
      setInModifica(null)
            ...
            <div key={reg.id} className="border border-gray-200 rounded-xl overflow-hidden bg-white">
              ...
              {isExpanded && (
                <div className="border-t border-gray-100 px-4 py-3 bg-gray-50 space-y-3">
                  ...
                  {!readOnly && inModifica === reg.id && formModifica && (
                    <div className="pt-3 mt-2 border-t border-gray-200">
                      <div className="grid gap-3" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))' }}>
                        {([
                          ['first_name', 'Nome'], ['last_name', 'Cognome'], ['email', 'Email'],
                          ['phone', 'Telefono'], ['company', 'Azienda'], ['job_title', 'Ruolo'],
                          ['dietary_requirements', 'Esigenze alimentari'],
                          ['accessibility_requirements', 'Accessibilita'],
                        ] as [keyof FormModifica, string][]).map(([campo, etichetta]) => (
                          <div key={campo}>
                            <label className="block text-gray-600 mb-1" style={{ fontSize: '12px' }}>{etichetta}</label>
                            <input
                              type={campo === 'email' ? 'email' : 'text'}
                              className="w-full border border-gray-300 rounded-lg px-2 py-1.5"
                              style={{ fontSize: '13px' }}
                              value={String(formModifica[campo] ?? '')}
                              onChange={e => setFormModifica(f => f ? { ...f, [campo]: e.target.value } : f)}
                            />
                          </div>
                        ))}
                      </div>

                      {Object.keys(formModifica.custom_answers).length > 0 && (
                        <div className="mt-3">
                          <div className="text-gray-600 mb-2" style={{ fontSize: '12px' }}>Risposte del modulo</div>
                          <div className="grid gap-3" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))' }}>
                            {Object.entries(formModifica.custom_answers).map(([chiave, valore]) => (
                              <div key={chiave}>
                                <label className="block text-gray-500 mb-1" style={{ fontSize: '12px' }}>{chiave}</label>
                                <input
                                  type="text"
                                  className="w-full border border-gray-300 rounded-lg px-2 py-1.5"
                                  style={{ fontSize: '13px' }}
                                  value={String(valore ?? '')}
                                  onChange={e => setFormModifica(f => f ? {
                                    ...f, custom_answers: { ...f.custom_answers, [chiave]: e.target.value }
                                  } : f)}
                                />
                              </div>
                            ))}
                      ...
                      <div className="flex gap-2 mt-3">
                        ...
                        <button
                          onClick={() => { setInModifica(null); setFormModifica(null) }}
                  ...
                  {!readOnly && (
                    <div className="flex flex-wrap gap-2 pt-2 border-t border-gray-200 items-center" style={{ fontSize: '13px' }}>
                      <button
                        disabled={isLoading}
                        onClick={() => (inModifica === reg.id ? (setInModifica(null), setFormModifica(null)) : apriModifica(reg))}
                        className="px-3 py-1.5 bg-gray-700 text-white rounded-lg hover:bg-gray-800 disabled:opacity-50 transition-colors"
                        style={{ fontSize: '13px' }}
                        title="Correggi i dati di questo iscritto"
                      >
                        {inModifica === reg.id ? 'Annulla modifica' : 'Modifica dati'}
                      </button>
