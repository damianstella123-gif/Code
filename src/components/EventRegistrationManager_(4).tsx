...
// Font disponibili: caricati da Google Fonts nella pagina pubblica.
const FONT_OPTIONS: { value: string; label: string; anteprima: string }[] = [
  { value: 'Inter', label: 'Inter (predefinito)', anteprima: "'Inter', sans-serif" },
  { value: 'Playfair Display', label: 'Playfair Display (elegante)', anteprima: "'Playfair Display', serif" },
  { value: 'Montserrat', label: 'Montserrat (moderno)', anteprima: "'Montserrat', sans-serif" },
  { value: 'Lora', label: 'Lora (classico)', anteprima: "'Lora', serif" },
  { value: 'Poppins', label: 'Poppins (informale)', anteprima: "'Poppins', sans-serif" },
  { value: 'Source Sans 3', label: 'Source Sans (istituzionale)', anteprima: "'Source Sans 3', sans-serif" },
]
...
function readThemeColors(theme: Record<string, unknown> | null | undefined): ThemeColors {
  return {
    ...
    font_family: typeof theme?.font_family === 'string' && theme.font_family.trim() !== '' ? theme.font_family : THEME_DEFAULTS.font_family,
    background_image_url: typeof theme?.background_image_url === 'string' ? theme.background_image_url : '',
...
function formattaPeriodo(inizio: string | null, fine: string | null): string {
  if (!inizio) return ''
  const fmtGiorno = new Intl.DateTimeFormat('it-IT', { day: 'numeric' })
  const fmtMeseAnno = new Intl.DateTimeFormat('it-IT', { day: 'numeric', month: 'long', year: 'numeric' })
  const d1 = new Date(inizio + 'T00:00:00')
  if (!fine || fine === inizio) return fmtMeseAnno.format(d1)
  const d2 = new Date(fine + 'T00:00:00')
  if (d1.getMonth() === d2.getMonth() && d1.getFullYear() === d2.getFullYear()) {
    return `${fmtGiorno.format(d1)} - ${fmtMeseAnno.format(d2)}`
  }
  return `${fmtMeseAnno.format(d1)} - ${fmtMeseAnno.format(d2)}`
}

// Costruisce una bozza di configurazione partendo dai dati gia presenti sull'evento,
// cosi chi crea il sito trova i campi compilati invece di una pagina bianca.
function precompilaDaEvento(ev: EventoBase | null, eventName: string): FormState {
  if (!ev) return { ...emptyForm, title: eventName, slug: normalizeRegistrationSlug(eventName) }

  // Il titolo dell'evento e spesso un codice tipo "#070": in quel caso usa la descrizione.
  const titoloGrezzo = (ev.title ?? '').trim()
  const isCodice = /^#?\d{1,4}$/.test(titoloGrezzo)
  const dalladescrizione = (ev.description ?? '').split(' - ')[0].trim()
  const titolo = (isCodice && dalladescrizione) ? dalladescrizione : (titoloGrezzo || eventName)

  const periodo = formattaPeriodo(ev.start_date, ev.end_date)
  const luogo = (ev.location ?? '').trim()
  const sottotitolo = [luogo, periodo].filter(Boolean).join(' \u00b7 ')

  // Chiusura iscrizioni: due settimane prima dell'inizio, se l'evento ha una data
  let chiusura = ''
  if (ev.start_date) {
    const d = new Date(ev.start_date + 'T23:59:00')
    d.setDate(d.getDate() - 14)
    d.setMinutes(d.getMinutes() - d.getTimezoneOffset())
    chiusura = d.toISOString().slice(0, 16)
  }

  return {
    ...emptyForm,
    title: titolo,
    slug: normalizeRegistrationSlug(titolo),
    subtitle: sottotitolo,
    capacity: ev.attendees ? String(ev.attendees) : '',
    closes_at: chiusura,
    confirmation_message: 'Grazie, la tua registrazione e stata confermata. Riceverai a breve una email con il riepilogo e il link per consultare il programma.',
  }
}

export default function EventRegistrationManager({ eventId, eventName, isArchived }: Props) {
  const [evento, setEvento] = useState<EventoBase | null>(null)
  ...
  const [soloInvitati, setSoloInvitati] = useState(false)
  const [invitatiOpen, setInvitatiOpen] = useState(false)
  const [invitati, setInvitati] = useState<{ id: string; email: string; nome: string | null; cognome: string | null; azienda: string | null; registrato: boolean }[]>([])
  const [testoInvitati, setTestoInvitati] = useState('')
  ...
  const load = useCallback(async () => {
    try {
      const [sites, perm, ev] = await Promise.all([
        fetchRegistrationSites(eventId),
        canManageRegistration(eventId).catch(() => false),
        Promise.resolve(
          supabase.from('events')
            .select('title, description, location, start_date, end_date, attendees, client')
            .eq('id', eventId).maybeSingle()
        ).then(r => (r.data as EventoBase | null) ?? null, () => null),
      ])
      setCanManage(perm)
      setEvento(ev)
      if (sites.length > 0) {
        ...
        if (s) {
          ...
          setSoloInvitati(!!s.solo_invitati)
  ...
  const caricaInvitati = useCallback(async () => {
    if (!site?.id) return
    const { data } = await supabase
      .from('registration_invitees')
      .select('id, email, nome, cognome, azienda, registrato')
      .eq('site_id', site.id)
      .order('email')
    if (Array.isArray(data)) setInvitati(data)
  }, [site?.id])

  useEffect(() => { if (invitatiOpen) caricaInvitati() }, [invitatiOpen, caricaInvitati])

  async function importaInvitati() {
    if (!site?.id || !testoInvitati.trim()) return
    setImportInCorso(true)
    try {
      const { data, error } = await supabase.rpc('importa_invitati', {
        p_site_id: site.id,
        p_testo: testoInvitati,
      })
      if (error) { showToast(error.message, 'error'); return }
      const r = data as { error?: string; aggiunti?: number; gia_presenti?: number; righe_non_valide?: number }
      if (r?.error) { showToast(r.error, 'error'); return }
      const parti = [`${r.aggiunti ?? 0} invitati aggiunti`]
      if (r.gia_presenti) parti.push(`${r.gia_presenti} gia in lista`)
      if (r.righe_non_valide) parti.push(`${r.righe_non_valide} righe senza email valida`)
      showToast(parti.join(', '), 'success')
      setTestoInvitati('')
      await caricaInvitati()
    } finally {
      setImportInCorso(false)
    }
  }

  async function rimuoviInvitato(id: string) {
    const { error } = await supabase.from('registration_invitees').delete().eq('id', id)
    if (error) { showToast(error.message, 'error'); return }
  ...
  // Link di anteprima: mostra il sito anche se non e ancora pubblicato,
  // protetto da un token cosi non e raggiungibile da chi non lo possiede.
  const previewUrl = () => {
    if (!site) return ''
    const t = site.preview_token
    return t
      ? `${PUBLIC_APP_URL}/r/${site.slug}?preview=${t}`
  ...
  const copyPreviewLink = () => {
    ...
    navigator.clipboard.writeText(u).then(
      () => showToast('Link di anteprima copiato: mostralo pure al cliente', 'success'),
        ...
        <div style={styles.emptyState}>
          ...
          {!readOnly && (
            <button
              ...
              onClick={() => { setForm(precompilaDaEvento(evento, eventName)); setEditing(true) }}
            >
        ...
        {site.status === 'published' && (
          <>
            ...
            <div style={styles.shareRow}>
              <button
                style={{ ...styles.btnOutline, borderColor: '#f59e0b', color: '#b45309' }}
                aria-label="Apri l'anteprima del sito"
                onClick={openPreview}
                title="Mostra la pagina come la vedra un partecipante, anche se non e ancora pubblicata"
              >
                <Eye size={14} /> Anteprima
              </button>
              <button
                style={{ ...styles.btnOutline, borderColor: '#f59e0b', color: '#b45309' }}
                aria-label="Copia il link di anteprima da mostrare al cliente"
                onClick={copyPreviewLink}
              >
                <Copy size={14} /> Link anteprima
              </button>
              <button
  ...
  return (
    <div style={styles.container}>
      ...
      >
        <div style={styles.formSection}>
          ...
          <div style={{ marginTop: 14 }}>
            <label style={styles.label}>Carattere</label>
            <select
              style={{ ...styles.input, maxWidth: 320 }}
              value={themeColors.font_family}
              onChange={e => setThemeColors(prev => ({ ...prev, font_family: e.target.value }))}
              disabled={readOnly}
            >
              {FONT_OPTIONS.map(f => (
            ...
            <p style={{ fontSize: 12, color: '#9ca3af', marginTop: 4 }}>
              Usa un carattere che si avvicini all'identita visiva del cliente.
            </p>
          </div>

          <div style={{ marginTop: 14 }}>
            <label style={styles.label}>Immagine di sfondo (facoltativa)</label>
            <input
              type="url"
              style={{ ...styles.input, maxWidth: 480 }}
              placeholder="https://..."
              value={themeColors.background_image_url}
              onChange={e => setThemeColors(prev => ({ ...prev, background_image_url: e.target.value }))}
              disabled={readOnly}
            />
            <p style={{ fontSize: 12, color: '#9ca3af', marginTop: 4 }}>
              Viene applicata dietro al modulo con una velatura, per non compromettere la leggibilita.
              Consigliata 1920x1080, sotto i 500 KB.
            </p>
          </div>

          <div style={{
            marginTop: 12, padding: 16, borderRadius: 8, border: '1px solid #e5e7eb',
            backgroundColor: themeColors.background_color,
            fontFamily: FONT_OPTIONS.find(f => f.value === themeColors.font_family)?.anteprima ?? 'inherit',
            backgroundImage: themeColors.background_image_url
              ? `linear-gradient(rgba(255,255,255,.82), rgba(255,255,255,.82)), url(${themeColors.background_image_url})`
              : undefined,
      ...
      {/* LISTA INVITATI */}
      <CollapsibleSection
        title={`Lista invitati${invitati.length > 0 ? ` (${invitati.length})` : ''}`}
        open={invitatiOpen}
        onToggle={() => setInvitatiOpen(o => !o)}
      >
        <p style={styles.sectionHint}>
          Se attivi la restrizione, potranno iscriversi solo le persone il cui indirizzo email
          e presente in questa lista. Utile per eventi su invito.
        </p>

        <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 14, marginBottom: 16, cursor: readOnly ? 'default' : 'pointer' }}>
          <input
            type="checkbox"
            checked={soloInvitati}
            onChange={e => setSoloInvitati(e.target.checked)}
            disabled={readOnly}
          />
          Consenti l'iscrizione solo agli indirizzi in lista
        </label>

        {soloInvitati && invitati.length === 0 && (
          <p style={{ fontSize: 13, color: '#b45309', background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 8, padding: 10, marginBottom: 16 }}>
            Attenzione: la restrizione e attiva ma la lista e vuota. Nessuno potra iscriversi
            finche non aggiungi almeno un indirizzo.
          </p>
        )}
        ...
        {!readOnly && (
          <div style={{ marginBottom: 18 }}>
            ...
            <textarea
              ...
              placeholder={'Un invitato per riga. Formati riconosciuti:\nmario.rossi@azienda.it\nmario.rossi@azienda.it, Mario, Rossi, Azienda Srl\nMario Rossi <mario.rossi@azienda.it>'}
              value={testoInvitati}
              onChange={e => setTestoInvitati(e.target.value)}
            />
            <p style={{ fontSize: 12, color: '#9ca3af', marginTop: 4, marginBottom: 8 }}>
              Puoi incollare direttamente da Excel o da una email: nome e azienda sono opzionali.
            </p>
            <button
              style={{ ...styles.btnPrimary, opacity: (!testoInvitati.trim() || importInCorso) ? 0.5 : 1 }}
              onClick={importaInvitati}
              disabled={!testoInvitati.trim() || importInCorso}
            >
              {importInCorso ? 'Importo...' : 'Aggiungi alla lista'}
            </button>
          </div>
        )}
        ...
        {invitati.length > 0 && (
          <div style={{ border: '1px solid #e5e7eb', borderRadius: 10, overflow: 'hidden' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 12px', background: '#f9fafb', fontSize: 12, color: '#6b7280', borderBottom: '1px solid #e5e7eb' }}>
              <span>{invitati.length} invitati</span>
              <span>{invitati.filter(i => i.registrato).length} gia iscritti</span>
            </div>
            <div style={{ maxHeight: 280, overflowY: 'auto' }}>
              {invitati.map(i => (
                <div key={i.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', borderBottom: '1px solid #f3f4f6', fontSize: 13 }}>
                  <span style={{
                    width: 8, height: 8, borderRadius: '50%', flexShrink: 0,
                    background: i.registrato ? '#22c55e' : '#d1d5db',
                  }} title={i.registrato ? 'Iscritto' : 'Non ancora iscritto'} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ color: '#111', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{i.email}</div>
                    {(i.nome || i.cognome || i.azienda) && (
                      <div style={{ fontSize: 12, color: '#9ca3af' }}>
                        {[[i.nome, i.cognome].filter(Boolean).join(' '), i.azienda].filter(Boolean).join(' \u00b7 ')}
                      </div>
                    )}
                  </div>
                  {!readOnly && !i.registrato && (
                    <button
                      onClick={() => rimuoviInvitato(i.id)}
                      style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9ca3af', padding: 4 }}
                      aria-label={`Rimuovi ${i.email}`}
                    >
                      <X size={14} />
                    </button>
                  )}
