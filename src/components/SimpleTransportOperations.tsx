...
export default function SimpleTransportOperations({ eventId, disabled }: Props) {
  ...
  const [tfServiceMode, setTfServiceMode] = useState<'disposizione' | 'tratta' | ''>('')
  const [tfServiceEndTime, setTfServiceEndTime] = useState('')
  const [editingMovement, setEditingMovement] = useState<TransportMovement | null>(null)
  ...
  const loadVehicles = useCallback(async (movementId: string, silent = false): Promise<VehicleRow[]> => {
    if (!silent) setLoadingVehicles(true)
    try {
      ...
      if (!silent) setLoadingVehicles(false)
    }
  }, [showToast])
  ...
  const loadPool = useCallback(async (movementId: string, silent = false) => {
  ...
  useEffect(() => {
    ...
    const unsub = subscribeTransportMovement(movementId, () => {
      loadVehicles(movementId, true)
      if (view === 'participants') {
        loadPool(movementId, true)
      }
    })
    unsubRef.current = unsub

    // Su mobile la connessione realtime può cadere quando lo schermo si blocca
    // o l'app va in background: al ritorno in primo piano ci riallineiamo
    // silenziosamente, per non perdere spunte fatte da altri nel frattempo.
    const handleVisible = () => {
  ...
  const goToParticipants = async (m: TransportMovement, v: VehicleRow) => {
    if (m.movement_status === 'draft') {
      try {
        await transitionTransportMovement(m.id, 'open')
        ...
        setMovements(fresh.filter(x => x.movement_status !== 'cancelled'))
      } catch (err: any) {
  ...
  const selectMovement = async (m: TransportMovement) => {
    setSelectedMovement(m)
    setSelectedVehicle(null)
    // Con un solo mezzo attivo si salta la scelta e si va dritti alla spunta.
    // Con più mezzi (o nessuno) resta la selezione: serve, ad es., quando il primo
    // mezzo si riempie e bisogna passare a un altro per lo stesso transfer.
    const rows = await loadVehiclesAndReturn(m.id)
    const attivi = rows.filter(v => v.operational_status !== 'cancelled')
    if (attivi.length === 1) {
      await goToParticipants(m, attivi[0])
    } else {
      setView('vehicles')
    }
  }
  ...
  const selectVehicle = async (v: VehicleRow) => {
    if (!selectedMovement) return
    await goToParticipants(selectedMovement, v)
  }

  const goBack = () => {
    if (view === 'participants') {
      const attivi = vehicles.filter(v => v.operational_status !== 'cancelled')
      if (attivi.length <= 1) {
        // Con un solo mezzo, tornare alla schermata di selezione non serve a
        // nulla: si andrebbe comunque dritti alla stessa spunta cliccandoci.
        goToList()
      } else {
        setSelectedVehicle(null)
        setView('vehicles')
  ...
  const openEditMovement = (m: TransportMovement) => {
    setEditingMovement(m)
    setTfOrigin(m.origin)
    setTfDestination(m.destination)
    setTfDepartureAt(m.departure_at ? isoToLocalDateTimeInput(m.departure_at) : '')
    setTfServiceMode(m.service_mode ?? '')
    setTfServiceEndTime(m.service_end_at ? isoToLocalDateTimeInput(m.service_end_at).slice(11) : '')
    setTfError('')
  ...
  const handleBoard = async (registrationId: string) => {
    ...
    try {
      ...
      await loadPool(selectedMovement.id, true)
      const updatedVehicles = await loadVehicles(selectedMovement.id, true)

      if (result.success) {
        const updated = updatedVehicles.find(v => v.id === vehicleId)
        const isNowFull = !!updated && updated.capacity != null && updated.boarded_count >= updated.capacity
        if (isNowFull) {
          showToast('Mezzo completo — torno alla lista trasporti.', 'success')
          goToList()
        }
      }
    } catch {
  ...
  const handleUnboard = async (assignmentId: string, participantName: string) => {
    ...
    try {
      ...
      await loadPool(selectedMovement.id, true)
      await loadVehicles(selectedMovement.id, true)
    } catch {
      ...
      <div style={containerStyle}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          ...
          <button
            ...
            onClick={() => { setShowTransferForm(true); setEditingMovement(null); setTfOrigin(''); setTfDestination(''); setTfDepartureAt(''); setTfServiceMode(''); setTfServiceEndTime(''); setTfDriverName(''); setTfDriverPhone(''); setTfVehicleType(''); setTfVehicleCapacity(''); setTfError('') }}
            disabled={disabled}
          >
            + Nuovo transfer
          </button>
        ...
        {showTransferForm && (
          <div style={inlineFormStyle}>
            <h4 style={{ margin: '0 0 12px' }}>{editingMovement ? 'Modifica Transfer' : 'Nuovo Transfer'}</h4>
            ...
            <div style={{ display: 'flex', gap: 8, margin: '4px 0' }}>
              <button
                ...
                style={{
                  ...
                  flex: 1,
                  background: tfServiceMode === 'tratta' ? 'var(--primary)' : undefined,
                  color: tfServiceMode === 'tratta' ? '#fff' : undefined,
                }}
                onClick={() => setTfServiceMode('tratta')}
              >
                A tratta
              </button>
              <button
                ...
                style={{
                  ...secondaryActionBtnStyle,
                  flex: 1,
                  background: tfServiceMode === 'disposizione' ? 'var(--primary)' : undefined,
                  color: tfServiceMode === 'disposizione' ? '#fff' : undefined,
                }}
                onClick={() => setTfServiceMode('disposizione')}
              >
                A disposizione
              </button>
            </div>
            {tfServiceMode === 'disposizione' && (
              <div>
                <label style={{ fontSize: 12, color: 'var(--text-secondary)', display: 'block', margin: '4px 0 4px' }}>
                  Fino alle ore
                </label>
                <input
                  style={inputStyle}
                  type="time"
                  value={tfServiceEndTime}
                  onChange={e => setTfServiceEndTime(e.target.value)}
                />
              </div>
            )}
            {!editingMovement && (
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
              <button
                ...
                onClick={async () => {
                  if (!editingMovement && !tfVehicleType) { setTfError('Selezionare una tipologia mezzo.'); return }
                  const parsedCap = Number.parseInt(tfVehicleCapacity, 10)
                  if (!editingMovement && (!tfVehicleCapacity || Number.isNaN(parsedCap) || parsedCap <= 0)) {
                  ...
                  try {
                    if (editingMovement) {
                      await saveTransportMovement({
                        movementId: editingMovement.id,
                        eventId,
                        label: editingMovement.label,
                        movementType: editingMovement.movement_type,
                        departureAt: localDateTimeToISO(tfDepartureAt),
                        origin: tfOrigin.trim(),
                        destination: tfDestination.trim(),
                        serviceMode: tfServiceMode || null,
                        serviceEndAt: tfServiceMode === 'disposizione' ? combineDateWithEndTime(tfDepartureAt, tfServiceEndTime) : null,
                      })
                      showToast('Transfer aggiornato', 'success')
                      setShowTransferForm(false)
                      setEditingMovement(null)
                      setTfOrigin(''); setTfDestination(''); setTfDepartureAt(''); setTfServiceMode(''); setTfServiceEndTime('')
                      setTfError('')
                      await loadMovements()
                      return
                    }
                    ...
                    showToast(err?.message ?? (editingMovement ? 'Errore nel salvataggio.' : 'Errore nella creazione.'), 'error')
                  } finally {
              ...
              <button
                ...
                onClick={() => { setShowTransferForm(false); setEditingMovement(null); setTfServiceEndTime('') }}
                disabled={savingTransfer}
              >
                Annulla
              </button>
          ...
          <div style={cardListStyle}>
            {activeMovements.map(m => (
              <div key={m.id} style={{ position: 'relative' }}>
                  ...
                  <div style={cardMetaStyle}>
                    {m.departure_at && (
                      <span>
                        ...
                        {formatDayLabel(m.departure_at)} {formatTime(m.departure_at)}
                      </span>
                    )}
                    {m.service_mode && (
                      <span style={{
                        ...
                        {m.service_mode === 'disposizione'
                          ? `A disposizione${m.service_end_at ? ` fino alle ${formatTime(m.service_end_at)}` : ''}`
                          : 'A tratta'}
                      </span>
                    )}
                  </div>
                  {m.drivers.length > 0 && (
                    <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 4, textAlign: 'left' }}>
                      {m.drivers.map((d, i) => (
                        <span key={i}>
                          {i > 0 && ' · '}
                          {d.name}
                ...
                <button
                  style={editTransferBtnStyle}
                  title="Modifica transfer"
                  onClick={(e) => { e.stopPropagation(); openEditMovement(m) }}
                  disabled={disabled}
                >
                  <Pencil size={16} />
                </button>
  ...
  if (view === 'participants' && selectedMovement && selectedVehicle) {
    const isFull = selectedVehicle.capacity != null && selectedVehicle.boarded_count >= selectedVehicle.capacity
    return (
      <div style={containerStyle}>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <button onClick={goBack} style={backBtnStyle} disabled={disabled}>
          ...
          <button
            onClick={goToList}
            style={isFull ? primaryActionBtnStyle : backBtnStyle}
            disabled={disabled}
          >
            Tutti i trasporti
          </button>
        ...
        {isFull && (
          <p style={{ fontSize: 13, color: 'var(--orange, #e67e22)', fontWeight: 600, margin: '8px 0 0' }}>
            Mezzo completo — seleziona un altro transfer o un altro mezzo.
          </p>
        )}
        ...
        {loadingPool ? (
          ...
          <div style={cardListStyle}>
            {pool.map(p => {
              ...
              const nonViene = p.tipo_transfer === '0'
              const vaInAuto = p.tipo_transfer === 'AUTO'
                ...
                <div key={p.registration_id} style={{ position: 'relative' }}>
                  <button
                    style={{
                      ...
                      background: isBoardedOnThis
                        ? 'rgba(47, 158, 104, 0.1)'
                        : nonViene
                          ? 'rgba(220, 53, 69, 0.08)'
                          : 'var(--panel)',
                      borderColor: isBoardedOnThis
                        ? 'var(--green, #2f9e68)'
                        : nonViene
                          ? 'var(--red, #dc3545)'
                          : 'var(--line)',
                  ...
                  >
                    <div style={{ flex: 1, minWidth: 0, textAlign: 'left' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                        <span style={{ fontSize: 14, fontWeight: 500, color: nonViene ? 'var(--red, #dc3545)' : 'var(--text)' }}>
                          {p.last_name} {p.first_name}
                        </span>
                        {nonViene && (
                          <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--red, #dc3545)', border: '1px solid var(--red, #dc3545)', borderRadius: 4, padding: '1px 6px' }}>
                            Non viene
                          </span>
                        )}
                        {vaInAuto && (
                          <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--orange, #e67e22)', border: '1px solid var(--orange, #e67e22)', borderRadius: 4, padding: '1px 6px' }}>
                            Auto propria
                          </span>
                        )}
                      </div>
...
function localDateTimeToISO(value: string): string | null {
  ...
  // "2026-09-17T20:30" (senza fuso) va interpretato come ora locale, non UTC:
  // new Date() lo fa correttamente per le stringhe con orario (a differenza
  // delle sole date), quindi basta poi serializzarlo in UTC per il database.
  const d = new Date(value)
  if (Number.isNaN(d.getTime())) return null
  return d.toISOString()
}

function combineDateWithEndTime(startLocalValue: string, endTime: string): string | null {
  if (!startLocalValue || !endTime) return null
  const datePart = startLocalValue.slice(0, 10)
  const startDate = new Date(startLocalValue)
  let endDate = new Date(`${datePart}T${endTime}`)
  if (Number.isNaN(startDate.getTime()) || Number.isNaN(endDate.getTime())) return null
  if (endDate <= startDate) {
    // La disposizione supera la mezzanotte: la fine va al giorno dopo.
...
function isoToLocalDateTimeInput(iso: string): string {
  // Percorso inverso: dal timestamp UTC salvato al valore "locale" (ora italiana)
  // che l'input datetime-local si aspetta, indipendentemente dal fuso del dispositivo.
  try {
...
function formatDayLabel(iso: string): string {
  try {
    const d = new Date(iso)
    const label = d.toLocaleDateString('it-IT', { weekday: 'short', day: 'numeric', timeZone: 'Europe/Rome' })
...
const editTransferBtnStyle: React.CSSProperties = {
  ...deleteTransferBtnStyle,
  right: 54,
}
