...
export default function RegistrationFieldsManager({ siteId, readOnly }: Props) {
  ...
  async function applyTemplate(sovrascrivi: boolean) {
    ...
    try {
      ...
      // Applica anche i testi informativi del modello, compilando i segnaposto
      // con i dati dell'evento. Le sezioni gia scritte non vengono toccate.
      let sezioni = 0
      const { data: cont } = await supabase.rpc('applica_contenuti_modello', {
        p_site_id: siteId,
        p_template_id: selectedTemplate.id,
        p_sovrascrivi: false,
      })
      const rc = cont as { sezioni_compilate?: number } | null
      if (rc?.sezioni_compilate) sezioni = rc.sezioni_compilate

      const parti = [`${res.campi_creati ?? 0} campi creati`]
      if (res.campi_gia_presenti) parti.push(`${res.campi_gia_presenti} gia presenti`)
      if (sezioni) parti.push(`${sezioni} sezioni di testo compilate`)
      showToast(`Modello applicato: ${parti.join(', ')}`, 'success')
                ...
                {templates.map(t => (
                  ...
                  >
                    ...
                    <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 6, fontStyle: 'italic' }}>
                      Include anche i testi informativi, compilati con luogo, date e referente dell'evento.
                    </div>
