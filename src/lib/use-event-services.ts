...
import { calcRowEconomics } from './event-economics'
export { calcRowEconomics, fetchAllEventsEconomics } from './event-economics'
export type { EventEconomicsSummary, RawRow } from './event-economics'
  ...
  const reload = useCallback(async () => {
    ...
    const sums: SupplierSummary[] = rawLinks.map(link => {
      ...
      for (const row of rows) {
        ...
        costo += econ.costo
        if (!econ.costo && !econ.venduto) hasMissingCosts = true
