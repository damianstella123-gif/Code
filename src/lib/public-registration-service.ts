...
export async function fetchPublicRegistrationSite(slug: string, previewToken?: string | null): Promise<PublicRegistrationSite | null> {
  const { data, error } = await supabase.rpc('get_public_registration_site', {
    p_slug: slug,
