...
export interface TransportMovement {
  ...
  service_mode: 'disposizione' | 'tratta' | null
...
export async function fetchTransportMovements(eventId: string): Promise<TransportMovement[]> {
  ...
  const movements = (data ?? []) as Omit<TransportMovement, 'drivers'>[]
  if (movements.length === 0) return []

  // Autisti: presi dai mezzi collegati, per mostrarli direttamente sulla scheda del transfer.
  const { data: vehiclesData, error: vehiclesError } = await supabase
    .from('transport_vehicles')
    .select('movement_id, driver_name, driver_phone, sort_order')
    .in('movement_id', movements.map(m => m.id))
    .neq('operational_status', 'cancelled')
    .order('sort_order', { ascending: true })

  if (vehiclesError) throwTranslated(vehiclesError)

  const driversByMovement = new Map<string, { name: string; phone: string }[]>()
  for (const v of (vehiclesData ?? []) as { movement_id: string; driver_name: string; driver_phone: string }[]) {
    if (!v.driver_name) continue
    const list = driversByMovement.get(v.movement_id) ?? []
    list.push({ name: v.driver_name, phone: v.driver_phone ?? '' })
    driversByMovement.set(v.movement_id, list)
  }

  return movements.map(m => ({ ...m, drivers: driversByMovement.get(m.id) ?? [] }))
}
...
export async function saveTransportMovement(input: {
  ...
  origin?: string
  destination?: string
  serviceMode?: 'disposizione' | 'tratta' | null
  ...
  const { data, error } = await supabase.rpc('save_transport_movement', {
    ...
    p_service_end_at: input.serviceEndAt ?? null,
  })
