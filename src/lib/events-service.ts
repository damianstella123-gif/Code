...
function rowToEvent(r: EventRow): Event {
  return {
    ...
    stato: r.status,
    statoLavoro: (r as { stato_lavoro?: Event['statoLavoro'] }).stato_lavoro ?? null,
    statoTermine: (r as { stato_termine?: Event['statoTermine'] }).stato_termine ?? null,
...
function eventToRow(e: Event): Omit<EventRow, 'created_at' | 'updated_at' | 'event_number'> {
  return {
    ...
    end_date: e.dataFine,
    status: e.stato,
    stato_lavoro: e.statoLavoro ?? null,
    stato_termine: e.statoTermine ?? null,
    budget: e.budget,
