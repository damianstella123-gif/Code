...
import { Users, Pencil, X, Plus, Phone, MessageCircle, Mail } from 'lucide-react'
...
// Contatti diretti: telefono, WhatsApp ed email di un collega.
function ContattiMembro({ email, telefono, nome }: { email?: string; telefono?: string | null; nome: string }) {
  const tel = (telefono ?? '').trim()
  const mail = (email ?? '').trim()
  if (!tel && !mail) return null

  const cifre = tel.replace(/[^\d+]/g, '')
  const wa = cifre.startsWith('+') ? cifre.slice(1)
    : cifre.startsWith('00') ? cifre.slice(2)
    : cifre.length >= 9 && !cifre.startsWith('39') ? '39' + cifre : cifre

  const stile: React.CSSProperties = {
    width: 30, height: 30, borderRadius: 8, display: 'inline-flex',
    alignItems: 'center', justifyContent: 'center', flexShrink: 0,
    border: '1px solid var(--line)', background: 'var(--panel2)', textDecoration: 'none',
  }

  return (
    <div className="flex items-center gap-1.5">
      {tel && (
        <a href={`tel:${cifre}`} style={stile} title={`Chiama ${nome}`} aria-label={`Chiama ${nome}`}>
          <Phone style={{ width: 14, height: 14, color: 'var(--blue)' }} />
        </a>
      )}
      {tel && wa.length >= 10 && (
        <a href={`https://wa.me/${wa}`} target="_blank" rel="noopener noreferrer" style={stile}
          title={`WhatsApp a ${nome}`} aria-label={`WhatsApp a ${nome}`}>
          <MessageCircle style={{ width: 14, height: 14, color: 'var(--green)' }} />
        </a>
      )}
      {mail && (
        <a href={`mailto:${mail}`} style={stile} title={`Email a ${nome}`} aria-label={`Email a ${nome}`}>
          <Mail style={{ width: 14, height: 14, color: 'var(--red2)' }} />
        </a>
      )}
    </div>
  )
      ...
      {responsabile && (
        <div className="panel p-4" style={{ border: '1px solid var(--red2)' }}>
          <div className="flex items-center gap-4">
            ...
            <ContattiMembro email={responsabile.email} telefono={responsabile.telefono} nome={responsabile.nome} />
            <button
      ...
      {teamMembers.filter(m => m.id !== event.responsabile).map(m => (
        <div key={m.id} className="panel p-4">
          <div className="flex items-center gap-4">
            ...
            <ContattiMembro email={m.email} telefono={m.telefono} nome={m.nome} />
            <button
