...
import {
  ...
  MoreVertical, Pencil, Play, Pause, Share2, Search
} from 'lucide-react'
...
type RisultatoRicerca = {
  ...
  trovato_in: 'nome' | 'categoria' | 'riassunto' | 'contenuto'
}
...
export function TabDocumenti({ event, isArchived }: { event: Event; isArchived?: boolean }) {
  ...
  async function loadFolders() {
    const { data } = await supabase
      ...
      .select('id, event_id, nome, parent_folder_id, created_at, sort_order, is_scartata')
      .eq('event_id', event.id)
      // L'ordine e esplicito: cliente in cima, poi le categorie di lavoro,
      // le opzioni scartate e l'archivio in fondo.
  ...
  useEffect(() => {
    ...
    if (q.length < 2) { setRisultati(null); setCercando(false); return }
    setCercando(true)
    const t = setTimeout(async () => {
      const { data, error } = await supabase.rpc('cerca_documenti_evento', {
        p_event_id: event.id, p_query: q, p_limite: 40,
      })
      setCercando(false)
      if (error) { setRisultati([]); return }
      const r = data as { risultati?: RisultatoRicerca[] } | null
  ...
  // Caricamento di un'intera cartella: ricrea la struttura di sottocartelle
  // leggendo il percorso relativo di ogni file e creando solo quelle mancanti.
  async function creaPercorsoCartelle(
    parti: string[],
    radice: string | null,
    ...
    for (const nome of parti) {
      chiave = chiave ? `${chiave}/${nome}` : nome
      const gia = cache.get(chiave)
      if (gia) { padre = gia; continue }
      ...
      let q = supabase
        ...
        .eq('event_id', event.id)
        .eq('nome', nome)
      q = padre === null ? q.is('parent_folder_id', null) : q.eq('parent_folder_id', padre)
      const { data: esistente } = await q.maybeSingle()

      if (esistente?.id) {
        cache.set(chiave, esistente.id)
        padre = esistente.id
        continue
      }
      ...
      const { data: nuova, error } = await supabase
        .from('document_folders')
        .insert({ event_id: event.id, nome, parent_folder_id: padre })
        .select('id')
        .single()

      if (error || !nuova) return padre
      cache.set(chiave, nuova.id)
      padre = nuova.id
    }
    return padre
  }
  ...
  async function handleUploadFolder(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    let caricati = 0
    let cartelleCreate = 0

    // Raggruppa i file per percorso, cosi ogni cartella viene creata una sola volta
    const perPercorso = new Map<string, File[]>()
    for (const f of files) {
      const rel = (f as File & { webkitRelativePath?: string }).webkitRelativePath || f.name
      const parti = rel.split('/').filter(Boolean)
      parti.pop()
      const chiave = parti.join('/')
      const arr = perPercorso.get(chiave) ?? []
      arr.push(f)
      perPercorso.set(chiave, arr)
    }

    for (const [percorso, elenco] of perPercorso) {
      const parti = percorso ? percorso.split('/') : []
      const prima = cache.size
      const destinazione = parti.length > 0
        ? await creaPercorsoCartelle(parti, currentFolderId, cache)
        : currentFolderId
      cartelleCreate += cache.size - prima
      await handleUploadFiles(elenco, destinazione, true)
      caricati += elenco.length
    }

    setUploading(false)
    await Promise.all([loadDocs(), loadFolders()])
    showToast(
      `${caricati} document${caricati === 1 ? 'o caricato' : 'i caricati'}` +
      (cartelleCreate ? ` in ${cartelleCreate} cartell${cartelleCreate === 1 ? 'a' : 'e'}` : ''),
      'success',
    )
  }

  async function handleUploadFiles(files: File[], targetFolderId: string | null, silenzioso = false) {
    ...
    processAnalysisQueue(uploadedDocIds)
  }

  // Analizza i documenti caricati a piccoli gruppi, invece che tutti insieme:
  // troppe richieste simultanee a document-ingest saturano il servizio di analisi
  // (vedi caso #081, ottobre 2026 — errore "servizio temporaneamente non disponibile").
  async function processAnalysisQueue(docIds: string[]) {
    const CONCORRENZA = 2
    const PAUSA_MS = 1500
    for (let i = 0; i < docIds.length; i += CONCORRENZA) {
      const gruppo = docIds.slice(i, i + CONCORRENZA)
      await Promise.all(gruppo.map((docId) => triggerAnalysis(docId)))
      if (i + CONCORRENZA < docIds.length) {
  ...
  // Il trascinamento di una cartella espone solo il nome tramite dataTransfer.files:
  // per leggerne il contenuto serve attraversare gli elementi ricorsivamente.
  type FileConPercorso = { file: File; percorso: string[] }

  async function leggiElementi(items: DataTransferItemList): Promise<FileConPercorso[]> {
    const risultato: FileConPercorso[] = []

    async function visita(entry: FileSystemEntry, percorso: string[]): Promise<void> {
      if (entry.isFile) {
        const f = await new Promise<File | null>(res =>
          (entry as FileSystemFileEntry).file(res, () => res(null)))
        if (f && !f.name.startsWith('.')) risultato.push({ file: f, percorso })
        return
      }
      if (entry.isDirectory) {
        const reader = (entry as FileSystemDirectoryEntry).createReader()
        // readEntries restituisce al massimo 100 elementi per volta: va richiamato
        // finche non torna vuoto, altrimenti le cartelle grandi vengono troncate.
        let lotto: FileSystemEntry[]
        do {
          lotto = await new Promise<FileSystemEntry[]>(res => reader.readEntries(res, () => res([])))
          for (const e of lotto) await visita(e, [...percorso, entry.name])
        } while (lotto.length > 0)
      }
    }

    const radici: FileSystemEntry[] = []
    ...
    for (const r of radici) {
      if (r.isFile) {
        const f = await new Promise<File | null>(res =>
          (r as FileSystemFileEntry).file(res, () => res(null)))
        if (f) risultato.push({ file: f, percorso: [] })
      } else {
        await visita(r, [])
      }
    }
  ...
  // Gestisce il rilascio: file singoli vanno diretti, le cartelle ricreano la struttura.
  async function gestisciRilascio(e: React.DragEvent, destinazione: string | null) {
    const items = e.dataTransfer.items
    if (!items || items.length === 0) {
      const files = Array.from(e.dataTransfer.files || [])
      if (files.length > 0) await handleUploadFiles(files, destinazione)
      return
    }

    setUploading(true)
    const elementi = await leggiElementi(items)
    if (elementi.length === 0) { setUploading(false); return }

    const cache = new Map<string, string>()
    const perPercorso = new Map<string, File[]>()
    for (const { file, percorso } of elementi) {
    ...
    for (const [percorso, elenco] of perPercorso) {
      const parti = percorso ? percorso.split('/') : []
      const prima = cache.size
      const dest = parti.length > 0
        ? await creaPercorsoCartelle(parti, destinazione, cache)
        : destinazione
      cartelleCreate += cache.size - prima
      await handleUploadFiles(elenco, dest, true)
      caricati += elenco.length
    }
  ...
  async function handlePreview(doc: EventDocument) {
    ...
    if (IMAGE_EXTS.includes(ext)) {
      ...
      // Il visualizzatore Microsoft richiede un indirizzo pubblico e stabile:
      // i nostri link sono temporanei e firmati, e viene rifiutato.
      // I file Office vengono quindi scaricati e aperti con l'applicazione del dispositivo.
      handleDownload(doc)
    } else {
      handleDownload(doc)
    }
  ...
  // Condivisione: genera un link temporaneo (24 ore) da inviare a un collega.
  // Su telefono usa il menu di condivisione del sistema, altrove copia negli appunti.
  async function handleShare(doc: EventDocument) {
    const { data, error } = await supabase.storage
      .from('documents')
    ...
    const testo = `${doc.nome || doc.file_name}\n${data.signedUrl}`

    if (navigator.share) {
      try {
        await navigator.share({ title: doc.nome || doc.file_name, text: testo })
        return
      } catch {
        // Condivisione annullata dall'utente: si ricade sulla copia negli appunti
      }
    }

    try {
      await navigator.clipboard.writeText(data.signedUrl)
      showToast('Link copiato. Resta valido 24 ore.', 'success')
    } catch {
  ...
  async function handleDelete(id: string) {
    ...
    // Il file nello storage viene rimosso per primo, ma un suo errore non deve
    // impedire la cancellazione del documento: segnalarlo basta.
    ...
    if (dbError) {
      showToast(`Impossibile eliminare "${doc.file_name}": ${dbError.message}`, 'error')
      return
    }
    if (count === 0) {
      showToast(`"${doc.file_name}" non e stato eliminato: permessi insufficienti su questo evento.`, 'error')
      return
    }
    ...
    showToast(
      storageError
        ? `Documento rimosso dall'elenco. Il file archiviato non e stato cancellato.`
        : `"${doc.file_name}" eliminato.`,
  ...
  return (
    <div className="space-y-4">
      {/* Ricerca: cerca nel nome, nella categoria e dentro al contenuto analizzato */}
      <div style={{ position: 'relative' }}>
        ...
        <input
          ...
          placeholder="Cerca per nome, cartella o contenuto del documento..."
          className="w-full text-sm"
          style={{
            height: 42, paddingLeft: 36, paddingRight: 36, borderRadius: 12,
            background: 'var(--panel-solid, var(--panel))', border: '1px solid var(--line)',
            color: 'var(--text)',
          }}
        />
        {ricerca && (
          <button onClick={() => setRicerca('')} aria-label="Annulla ricerca"
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <label className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all cursor-pointer"
              ...
              <input type="file" className="hidden" onChange={handleUpload} multiple disabled={uploading}
                accept=".pdf,.xlsx,.xls,.pptx,.ppt,.docx,.jpg,.jpeg,.png,.msg,.eml,.mp3,.wav,.mp4,.mov,.webm" />
            </label>
            <label className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all cursor-pointer"
              style={{ background: 'color-mix(in srgb, var(--blue) 12%, transparent)', color: 'var(--blue)', border: '1px solid var(--blue)' }}
              title="Carica una o piu cartelle: le sottocartelle vengono ricreate automaticamente">
              <FolderOpen className="w-3.5 h-3.5" />
              {uploading ? 'Caricamento...' : 'Carica cartella'}
              <input
                type="file"
                className="hidden"
      ...
      {/* Risultati della ricerca: sostituiscono cartelle ed elenco finche si cerca */}
      {risultati !== null && (
        <div className="space-y-2">
          <p className="text-xs" style={{ color: 'var(--muted)' }}>
            {cercando ? 'Ricerca in corso...'
              : risultati.length === 0 ? `Nessun documento trovato per "${ricerca}"`
              : `${risultati.length} document${risultati.length === 1 ? 'o trovato' : 'i trovati'}`}
          </p>

          {risultati.map(r => (
            <div key={r.id} className="panel p-3" style={{ position: 'relative' }}>
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 flex-wrap mt-0.5">
                    {r.cartella && (
                      <span className="text-[11px] flex items-center gap-1" style={{ color: 'var(--muted)' }}>
                        <Folder className="w-3 h-3" /> {r.cartella}
                      </span>
                    )}
                    <span className="text-[11px] px-1.5 py-0.5 rounded"
                      style={{ background: 'var(--panel2)', color: 'var(--muted)' }}>
                      trovato nel {r.trovato_in}
                    </span>
                  </div>
                  ...
                  {r.summary && r.trovato_in !== 'contenuto' && (
                    <p className="text-xs mt-1.5 leading-relaxed" style={{ color: 'var(--muted)' }}>
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  <button
                    onClick={() => { const d = docs.find(x => x.id === r.id); if (d) handlePreview(d) }}
                    className="p-2 rounded-lg hover:bg-[var(--line)]" title="Apri">
                    <Eye className="w-4 h-4" style={{ color: 'var(--blue)' }} />
                  </button>
                  <button
                    onClick={() => { setRicerca(''); setCurrentFolderId(r.folder_id) }}
                    className="p-2 rounded-lg hover:bg-[var(--line)]" title="Vai alla cartella">
                    <FolderOpen className="w-4 h-4" style={{ color: 'var(--muted)' }} />
                  </button>
                </div>
              </div>
            </div>
      ...
      {currentFolderId && (
        <nav className="flex items-center gap-1 text-xs flex-wrap" style={{ color: 'var(--muted)' }}>
          <button
            ...
            onDrop={async e => { e.preventDefault(); setDragOverBreadcrumb(null); const docId = e.dataTransfer.getData('text/document-id'); if (docId) { handleMoveDoc(docId, null); return }; await gestisciRilascio(e, null) }}
            ...
            <span key={f.id} className="flex items-center gap-1">
              ...
              {i === folderPath.length - 1 ? (
                <span
                  ...
                  onDragLeave={() => setDragOverBreadcrumb(null)}
                  onDrop={async e => { e.preventDefault(); setDragOverBreadcrumb(null); const docId = e.dataTransfer.getData('text/document-id'); if (docId) { handleMoveDoc(docId, f.id); return }; await gestisciRilascio(e, f.id) }}
          ...
          {currentFolders.map(folder => {
            ...
            return (
              <div
                ...
                onDrop={async e => { e.preventDefault(); setDragOverFolderId(null); const docId = e.dataTransfer.getData('text/document-id'); if (docId) { handleMoveDoc(docId, folder.id); return }; await gestisciRilascio(e, folder.id) }}
              >
                  ...
                  <div className="flex items-center gap-2 min-w-0 flex-1">
                    ...
                    {isRenaming ? (
                      ...
                      <span className="text-sm font-medium truncate flex items-center gap-1.5"
                        style={{ color: folder.is_scartata ? 'var(--muted)' : 'var(--text)' }}>
                        {folder.nome}
                        {folder.is_scartata && (
                          <span
                            title="Opzione valutata e non scelta"
                            style={{
                              fontSize: 10, fontWeight: 500, padding: '1px 6px', borderRadius: 999,
                              background: 'var(--panel2)', color: 'var(--muted)', flexShrink: 0,
                            }}>
                            scartata
                          </span>
                        )}
      ...
      <div
        ...
        onDrop={async e => { if (!hasExternalFiles(e)) return; e.preventDefault(); externalDragCounter.current = 0; setExternalDragOver(false); await gestisciRilascio(e, currentFolderId) }}
      >
        {externalDragOver && (
      ...
      {previewUrl && (
        <div className="fixed inset-0 z-50 flex flex-col" style={{ background: 'rgba(0,0,0,0.9)' }}>
          <div className="flex items-center gap-2 px-3 py-2 flex-shrink-0"
            ...
            <p className="text-sm font-medium truncate flex-1 min-w-0" style={{ color: 'var(--text)' }}>{previewName}</p>
            <a href={previewUrl} target="_blank" rel="noopener noreferrer"
              className="p-2 rounded-lg hover:bg-[var(--line)]" title="Apri in una nuova scheda">
              <ExternalLink className="w-5 h-5" style={{ color: 'var(--blue)' }} />
            </a>
            <button onClick={() => { setPreviewUrl(null); setPreviewName(''); setPreviewType(null) }}
              className="p-2 rounded-lg hover:bg-[var(--line)]" aria-label="Chiudi anteprima">
              <X className="w-5 h-5" style={{ color: 'var(--muted)' }} />
            </button>
          </div>
          {/* L'area occupa tutto lo spazio rimasto: su telefono l'anteprima
              deve riempire lo schermo, non fermarsi a meta. */}
          <div className="flex-1 min-h-0 flex items-center justify-center overflow-auto p-0 sm:p-4">
            {previewType === 'image' && (
              <img src={previewUrl} alt={previewName} className="max-w-full max-h-full object-contain sm:rounded-lg" />
            )}
            {previewType === 'pdf' && (
              <iframe src={previewUrl} title={previewName}
                style={{ width: '100%', height: '100%', border: 'none', maxWidth: 900 }}
...
function DocumentRow({
  ...
  onPreview, onDownload, onDelete, onShare, onAnalyze, onMove,
  ...
  onShare: (doc: EventDocument) => void
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          ...
          {canManageDocs && (
            <div className="relative" ref={docMenuOpen === doc.id ? (docMenuRef as React.RefObject<HTMLDivElement>) : undefined}>
              ...
              {docMenuOpen === doc.id && (
                <div className="absolute right-0 top-full mt-1 min-w-[170px] rounded-lg py-1 shadow-xl"
                  style={{ background: 'var(--panel-solid, var(--panel))', border: '1px solid var(--line)', zIndex: 1000 }}>
