...
import { ArrowDownLeft, ArrowUpRight, Phone, MessageCircle, Mail } from 'lucide-react'
...
// Pulsanti per contattare direttamente una persona: telefono, WhatsApp, email.
// WhatsApp richiede il numero in formato internazionale senza spazi ne simboli.
function ContattiRapidi({ email, telefono, nome, compatto }: {
  email?: string | null
  ...
  const soloCifre = tel.replace(/[^\d+]/g, '')
  const perWhatsapp = soloCifre.startsWith('+')
    ? soloCifre.slice(1)
    : soloCifre.startsWith('00')
      ? soloCifre.slice(2)
      : soloCifre.length >= 9 && !soloCifre.startsWith('39')
        ? '39' + soloCifre
        : soloCifre

  const dim = compatto ? 28 : 34
  const stile: React.CSSProperties = {
    width: dim, height: dim, borderRadius: 9, display: 'inline-flex',
  ...
  return (
    <div className="flex items-center gap-1.5" onClick={e => e.stopPropagation()}>
      {tel && (
        <a href={`tel:${soloCifre}`} style={stile} title={`Chiama ${nome ?? ''}`} aria-label={`Chiama ${nome ?? 'contatto'}`}>
          <Phone style={{ width: compatto ? 13 : 15, height: compatto ? 13 : 15, color: 'var(--blue)' }} />
        </a>
      )}
      {tel && perWhatsapp.length >= 10 && (
        <a href={`https://wa.me/${perWhatsapp}`} target="_blank" rel="noopener noreferrer"
          style={stile} title={`Scrivi su WhatsApp a ${nome ?? ''}`} aria-label={`WhatsApp ${nome ?? 'contatto'}`}>
          <MessageCircle style={{ width: compatto ? 13 : 15, height: compatto ? 13 : 15, color: 'var(--green)' }} />
        </a>
      )}
      {mail && (
        <a href={`mailto:${mail}`} style={stile} title={`Scrivi a ${mail}`} aria-label={`Email ${nome ?? 'contatto'}`}>
          <Mail style={{ width: compatto ? 13 : 15, height: compatto ? 13 : 15, color: 'var(--red2)' }} />
        </a>
      )}
    </div>
  )
...
export function TabOverview({ event, progress, completedTasks, totalTasks, budgets, clients, onClientClick, internalUsers, comunicazioni }: {
  ...
  const altriMembri = (event.team ?? []).filter(id => id !== event.responsabile)
      ...
      {cliente && (
        ...
        >
          ...
          <div>
            ...
            {(cliente as { reparto?: string }).reparto && (
              <p className="text-sm mt-0.5" style={{ color: 'var(--muted)' }}>
                Reparto: {(cliente as { reparto?: string }).reparto}
              </p>
            )}
          </div>
          <div className="flex items-center justify-between gap-3 mt-3">
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: 'var(--muted)', opacity: 0.6 }}>CLICCA PER APRIRE NEL CRM</p>
            <ContattiRapidi email={cliente.email} telefono={cliente.telefono} nome={cliente.referente || cliente.nome} />
          </div>
        </div>
      )}

      {/* Responsabile dell'evento, accanto al cliente: stesso peso visivo,
          con i contatti diretti a portata di mano. */}
      {responsabile && (
        <div style={{ background: 'var(--panel-solid)', border: '1px solid var(--line)', borderRadius: '14px', padding: '20px' }}>
          <p style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.04em', color: 'var(--muted)', marginBottom: '12px' }}>Responsabile</p>
          <div className="flex items-center gap-3">
            <img
              src={responsabile.avatar}
              alt=""
              style={{ width: 42, height: 42, borderRadius: '50%', objectFit: 'cover', flexShrink: 0, border: '1px solid var(--line)' }}
            />
            <div className="min-w-0 flex-1">
              <p style={{ fontFamily: 'var(--font-serif)', fontWeight: 600, color: 'var(--text)' }}>{responsabile.nome}</p>
              {responsabile.email && (
                <p className="text-sm mt-0.5 truncate" style={{ color: 'var(--muted)' }}>{responsabile.email}</p>
              )}
              {responsabile.telefono && (
                <p className="text-sm mt-0.5" style={{ color: 'var(--muted)' }}>{responsabile.telefono}</p>
              )}
            </div>
          </div>
          <div className="flex items-center justify-between gap-3 mt-3">
            {altriMembri.length > 0 && (
              <p style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: 'var(--muted)', opacity: 0.6 }}>
                + {altriMembri.length} NEL TEAM
              </p>
            )}
            <div className="ml-auto">
              <ContattiRapidi email={responsabile.email} telefono={responsabile.telefono} nome={responsabile.nome} />
            </div>
          </div>
        </div>
      )}
