...
import type { TabId, InternalUser } from './eventi/shared-types'
...
// Stati del metodo di lavoro interno, come nel foglio PRATICHE
const STATI_LAVORO = ['Tutti', 'CONFERMATO', 'NON CONFERMATO', 'PREVENTIVO']
const STATI_TERMINE = ['DA DEFINIRE', 'IN CORSO', 'TERMINATO']

function statoColor(stato: string) {
  switch (stato) {
    case 'CONFERMATO': return 'var(--green)'
    case 'NON CONFERMATO': return 'var(--red2)'
    case 'PREVENTIVO': return 'var(--yellow)'
    case 'TERMINATO': return 'var(--blue)'
    case 'IN CORSO': return 'var(--green)'
    case 'DA DEFINIRE': return 'var(--muted)'
    default: return 'var(--muted)'
  }
}

function statoLabel(stato: string) {
  return stato === 'NON CONFERMATO' ? 'NON CONF.' : stato
}
...
interface EventDetailProps {
  ...
  onStatusChange: (event: Event, newStato: string) => void
  budgets: Uscita[]
  suppliers: Supplier[]
...
function EventDetail({ event, isArchived, onBack, onEdit, onDelete, onArchive, onStatusChange, budgets, suppliers, comunicazioni, internalUsers, clients, onSuppliersChanged }: EventDetailProps) {
  ...
  const statiSequenza = STATI_TERMINE
  const currentIdx = statiSequenza.indexOf(event.statoTermine ?? '')
  ...
  const statoBadge = (() => {
    switch (event.statoLavoro) {
      case 'CONFERMATO': return { color: 'var(--green)', bg: 'color-mix(in srgb, var(--green) 14%, transparent)' }
      case 'NON CONFERMATO': return { color: 'var(--red2)', bg: 'color-mix(in srgb, var(--red2) 12%, transparent)' }
      case 'PREVENTIVO': return { color: 'var(--yellow)', bg: 'color-mix(in srgb, var(--yellow) 16%, transparent)' }
      default: return { color: 'var(--muted)', bg: 'var(--line)' }
    }
  })()

  const termineBadge = (() => {
    switch (event.statoTermine) {
      case 'TERMINATO': return { color: 'var(--blue)', bg: 'color-mix(in srgb, var(--blue) 12%, transparent)' }
      case 'IN CORSO': return { color: 'var(--green)', bg: 'color-mix(in srgb, var(--green) 12%, transparent)' }
      case 'DA DEFINIRE': return { color: 'var(--muted)', bg: 'var(--line)' }
      default: return null
    }
  })()
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <span style={{
              ...
              {event.statoLavoro ? statoLabel(event.statoLavoro) : 'DA CLASSIFICARE'}
            </span>
            {termineBadge && event.statoTermine && (
              <span style={{
                fontFamily: 'var(--font-mono)', fontSize: '9px', fontWeight: 600,
                letterSpacing: '0.04em', textTransform: 'uppercase',
                padding: '3px 8px', borderRadius: '4px',
                color: termineBadge.color, background: termineBadge.bg,
              }}>
                {event.statoTermine}
              </span>
            )}
...
export default function Eventi() {
  ...
  const handleStatusChange = useCallback(async (event: Event, newStato: string) => {
    let remote: Event | null = null
    try {
      remote = await updateEventRemote(event.id, { statoTermine: newStato as Event['statoTermine'] })
    } catch (err: any) {
  ...
  const filtered = useMemo(() => {
    return visibleEvents.filter(e => {
      ...
      const matchStato = filterStato === 'Tutti' || e.statoLavoro === filterStato
  ...
  const statoBadgeStyle = (stato: string): React.CSSProperties => {
    switch (stato) {
      case 'CONFERMATO': return { color: 'var(--green)', background: 'color-mix(in srgb, var(--green) 14%, transparent)' }
      case 'NON CONFERMATO': return { color: 'var(--red2)', background: 'color-mix(in srgb, var(--red2) 12%, transparent)' }
      case 'PREVENTIVO': return { color: 'var(--yellow)', background: 'color-mix(in srgb, var(--yellow) 18%, transparent)' }
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '12px', marginTop: '18px' }}>
          {filtered.map((event) => {
            ...
            return (
              ...
              >
                {/* Logo del cliente in filigrana: identifica la scheda a colpo d'occhio
                    senza interferire con la leggibilita del contenuto. */}
                {cliente?.logoUrl && (
                  <div
                    aria-hidden="true"
                    style={{
                      position: 'absolute', inset: 0, pointerEvents: 'none',
                      backgroundImage: `url(${cliente.logoUrl})`,
                      backgroundRepeat: 'no-repeat',
                      backgroundPosition: 'right -20px bottom -20px',
                      backgroundSize: '150px auto',
                      opacity: 0.06,
                      maskImage: 'linear-gradient(to left, rgba(0,0,0,1), rgba(0,0,0,0) 70%)',
                      WebkitMaskImage: 'linear-gradient(to left, rgba(0,0,0,1), rgba(0,0,0,0) 70%)',
                    }}
                  />
                )}
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'relative' }}>
                  <span style={{
                    ...
                    letterSpacing: '0.04em', textTransform: 'uppercase',
                    padding: '3px 8px', borderRadius: '4px',
                    ...statoBadgeStyle(event.statoLavoro ?? ''),
                  }}>
                    {event.statoLavoro ? statoLabel(event.statoLavoro) : 'DA CLASSIFICARE'}
                  </span>
                  <span style={{
