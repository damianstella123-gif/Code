...
export default function Performance() {
  ...
  const [tab, setTab] = useState<'eventi' | 'synergy' | 'tempo'>('synergy')
    ...
    <div>
      <div className="wire-card-flat" style={{ padding: '16px', marginBottom: '20px', borderRadius: 12, border: '1px solid var(--line)' }}>
        ...
        <div className="flex gap-1" style={{ marginTop: 12 }}>
          {([['eventi', 'Eventi ROI'], ['synergy', 'Synergy Impact'], ['tempo', 'Tempo di lavoro']] as const).map(([id, label]) => (
            <button
              key={id}
              onClick={() => setTab(id)}
              style={{
...
type GiornoLavoro = { data: string; minuti: number; eventi: number; dalle: string; alle: string }
type PersonaLavoro = {
  user_id: string
  nome: string
...
function TempoLavoroSection() {
  const [persone, setPersone] = useState<PersonaLavoro[]>([])
  ...
  useEffect(() => {
    ...
    setCaricamento(true)
    const dal = new Date()
    dal.setDate(dal.getDate() - periodo)
    supabase
      .rpc('tempo_lavoro_giornaliero', { p_dal: dal.toISOString().slice(0, 10), p_al: null, p_user_id: null })
      .then(({ data }) => {
        if (annullato) return
        const r = data as { per_persona?: PersonaLavoro[] } | null
        setPersone(r?.per_persona ?? [])
        setCaricamento(false)
      })
    return () => { annullato = true }
  }, [periodo])

  const totaleOre = persone.reduce((acc, p) => acc + p.minuti_totali, 0) / 60

  return (
    <div>
      <div className="flex items-center justify-between flex-wrap gap-3" style={{ marginBottom: 16 }}>
        <div className="flex gap-1">
          {([[7, 'Ultima settimana'], [30, 'Ultimo mese'], [90, 'Ultimi 3 mesi']] as const).map(([g, label]) => (
            <button
              key={g}
              onClick={() => setPeriodo(g)}
              style={{
                fontFamily: 'var(--font-mono)', fontSize: 11, padding: '6px 12px', borderRadius: 8,
                border: '1px solid var(--line)', cursor: 'pointer',
                background: periodo === g ? 'var(--text)' : 'transparent',
                color: periodo === g ? 'var(--panel-solid)' : 'var(--muted)',
              }}
            >
              {label}
            </button>
        ...
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--muted)' }}>
          {totaleOre.toFixed(1)} ore complessive
        </span>
      </div>

      <p style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 16, lineHeight: 1.5 }}>
        Misura il tempo con una scheda evento aperta e la finestra in primo piano.
        Non comprende il lavoro svolto fuori dalle schede evento.
      </p>

      {caricamento && (
        <p style={{ fontSize: 13, color: 'var(--muted)' }}>Caricamento...</p>
      )}

      {!caricamento && persone.length === 0 && (
        <p style={{ fontSize: 13, color: 'var(--muted)' }}>Nessuna attività registrata nel periodo.</p>
      )}

      <div style={{ display: 'grid', gap: 10 }}>
        {persone.map(p => {
          const aperta = espansa === p.user_id
          const massimo = Math.max(...p.giorni.map(g => g.minuti), 1)
          return (
            <div key={p.user_id}
              style={{ background: 'var(--panel-solid)', border: '1px solid var(--line)', borderRadius: 12, overflow: 'hidden' }}>
              <button
                onClick={() => setEspansa(aperta ? null : p.user_id)}
                style={{ width: '100%', padding: 14, background: 'transparent', border: 'none', cursor: 'pointer', textAlign: 'left' }}
              >
                <div className="flex items-center justify-between gap-3 flex-wrap">
                  <span style={{ fontWeight: 600, color: 'var(--text)' }}>{p.nome}</span>
                  <div className="flex items-center gap-4" style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--muted)' }}>
                    <span><strong style={{ color: 'var(--text)', fontSize: 14 }}>{p.ore_totali}</strong> ore</span>
                    <span>{p.giorni_attivi} giorni</span>
                    <span>media {formatDurata(p.media_minuti_giorno)}</span>
                  </div>
                </div>

                {/* Andamento giornaliero, dal piu recente */}
                <div className="flex items-end gap-1" style={{ marginTop: 10, height: 34 }}>
                  {[...p.giorni].reverse().map(g => (
                    <div
                      key={g.data}
                      title={`${g.data}: ${formatDurata(g.minuti)}`}
                      style={{
                        flex: 1, minWidth: 3, maxWidth: 14,
                        height: `${Math.max(3, (g.minuti / massimo) * 34)}px`,
                        background: 'var(--blue)', borderRadius: 2, opacity: 0.75,
                      }}
                    />
                  ))}
                </div>
                ...
                <div style={{ borderTop: '1px solid var(--line)', padding: '4px 0' }}>
                  {p.giorni.map(g => (
                    <div key={g.data}
                      ...
                      style={{ padding: '8px 14px', fontSize: 13, borderBottom: '1px solid var(--line)' }}>
                      <span style={{ color: 'var(--text)' }}>
                        {new Date(g.data + 'T00:00:00').toLocaleDateString('it-IT', { weekday: 'short', day: 'numeric', month: 'short' })}
                      </span>
                      <div className="flex items-center gap-4" style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--muted)' }}>
                        <span>{g.dalle} – {g.alle}</span>
                        <span>{g.eventi} event{g.eventi === 1 ? 'o' : 'i'}</span>
                        <strong style={{ color: 'var(--text)' }}>{formatDurata(g.minuti)}</strong>
                      </div>
                    </div>
                  ))}
                </div>
