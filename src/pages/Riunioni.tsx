...
const STATO_LAVORO_VALORI = ['PREVENTIVO', 'CONFERMATO', 'NON CONFERMATO'] as const
const STATO_TERMINE_VALORI = ['DA DEFINIRE', 'IN CORSO', 'TERMINATO'] as const
...
// Stati del metodo di lavoro interno: commerciale e avanzamento, come nel foglio PRATICHE
function StatoLavoroBadge({ valore }: { valore?: string | null }) {
  if (!valore) return null
  const colori: Record<string, { bg: string; fg: string }> = {
    'CONFERMATO': { bg: '#dcfce7', fg: '#166534' },
    'NON CONFERMATO': { bg: '#fee2e2', fg: '#991b1b' },
    'PREVENTIVO': { bg: '#fef3c7', fg: '#92400e' },
  }
  const c = colori[valore] ?? { bg: 'var(--panel2)', fg: 'var(--muted)' }
  return (
    <span style={{ padding: '2px 8px', fontSize: 11, borderRadius: 6, background: c.bg, color: c.fg, fontWeight: 500, whiteSpace: 'nowrap' }}>
      {valore === 'NON CONFERMATO' ? 'NON CONF.' : valore}
    </span>
  )
}

function StatoTermineBadge({ valore }: { valore?: string | null }) {
  if (!valore) return null
  const colori: Record<string, { bg: string; fg: string }> = {
    'TERMINATO': { bg: '#e0e7ff', fg: '#3730a3' },
    'IN CORSO': { bg: '#cffafe', fg: '#155e75' },
    'DA DEFINIRE': { bg: 'var(--panel2)', fg: 'var(--muted)' },
  }
  const c = colori[valore] ?? { bg: 'var(--panel2)', fg: 'var(--muted)' }
  return (
    <span style={{ padding: '2px 8px', fontSize: 11, borderRadius: 6, background: c.bg, color: c.fg, fontWeight: 500, whiteSpace: 'nowrap' }}>
...
function useEventsFilters(events: Event[]) {
  ...
  const [filterLavoro, setFilterLavoro] = useState('')
  const [filterTermine, setFilterTermine] = useState('')
  ...
  const months = useMemo(() => {
    ...
    const unici = [...new Set(filtered.map(e => meseLabel(e.dataInizio)).filter(Boolean))]
    return unici.sort((a, b) => {
      const [ma, aa] = a.split(' ')
      const [mb, ab] = b.split(' ')
      if (aa !== ab) return Number(aa) - Number(ab)
      return MESI.indexOf(ma) - MESI.indexOf(mb)
    })
  }, [events, filterYear])
  ...
  const sorted = useMemo(() => {
    ...
    if (filterLavoro) list = list.filter(e => e.statoLavoro === filterLavoro)
    if (filterTermine) list = list.filter(e => e.statoTermine === filterTermine)
    if (filterMonth) list = list.filter(e => meseLabel(e.dataInizio) === filterMonth)
    list.sort((a, b) => {
      // Ordinamento per data: prima la data di inizio, a parita quella di fine.
      // Gli eventi senza data finiscono in fondo, non in cima.
      if (sortField === 'dataInizio' || sortField === 'dataFine') {
        const ai = a.dataInizio || ''
        const bi = b.dataInizio || ''
        if (!ai && !bi) return 0
        if (!ai) return 1
        if (!bi) return -1
        let cmp = ai.localeCompare(bi)
        if (cmp === 0) cmp = (a.dataFine || '').localeCompare(b.dataFine || '')
    ...
    return list
  }, [events, filterStatus, filterLavoro, filterTermine, filterMonth, filterYear, sortField, sortDir])
  ...
  return { sorted, sortField, sortDir, toggleSort, filterStatus, setFilterStatus, filterLavoro, setFilterLavoro, filterTermine, setFilterTermine, filterMonth, setFilterMonth, filterYear, setFilterYear, years, months }
}

// ─── Filter Bar ──────────────────────────────────────────────────────────────

function FilterBar({ filterLavoro, setFilterLavoro, filterTermine, setFilterTermine, filterMonth, setFilterMonth, filterYear, setFilterYear, years, months, count }: {
  filterLavoro: string; setFilterLavoro: (v: string) => void
  filterTermine: string; setFilterTermine: (v: string) => void
  ...
  return (
    <div className="flex gap-3 flex-wrap items-center" style={{ padding: '12px 16px', borderBottom: '1px solid var(--line)' }}>
      ...
      <select value={filterLavoro} onChange={e => setFilterLavoro(e.target.value)} style={selectStyle}>
        <option value="">Tutti gli stati</option>
        <option value="CONFERMATO">Confermato</option>
        <option value="NON CONFERMATO">Non confermato</option>
        <option value="PREVENTIVO">Preventivo</option>
      </select>
      <select value={filterTermine} onChange={e => setFilterTermine(e.target.value)} style={selectStyle}>
        <option value="">Tutti gli avanzamenti</option>
...
function EditableStatus({ ev, eventId, onSaved }: { ev: Event; eventId: string; onSaved: () => void }) {
  const { saving, saved, error, save } = useCellSave(eventId, onSaved)
  const sel: React.CSSProperties = {
    fontSize: '0.7rem', padding: '2px 5px', border: '1px solid var(--line)',
    borderRadius: 6, background: 'var(--panel2)', color: 'var(--text)', cursor: 'pointer',
  }

  return (
    <div className="flex items-center gap-1 flex-wrap" onClick={e => e.stopPropagation()}>
      <select value={ev.statoLavoro ?? ''} disabled={saving} style={sel}
        onChange={async e => { try { await save({ statoLavoro: (e.target.value || null) as Event['statoLavoro'] }) } catch {} }}>
        <option value="">—</option>
        {STATO_LAVORO_VALORI.map(v => <option key={v} value={v}>{v}</option>)}
      </select>
      <select value={ev.statoTermine ?? ''} disabled={saving} style={sel}
        onChange={async e => { try { await save({ statoTermine: (e.target.value || null) as Event['statoTermine'] }) } catch {} }}>
        <option value="">—</option>
        {STATO_TERMINE_VALORI.map(v => <option key={v} value={v}>{v}</option>)}
      </select>
      <SaveIndicator saving={saving} saved={saved} error={error} />
    </div>
  )
            ...
            {filters.sorted.map(ev => (
              <tr key={ev.id} onClick={() => !adminEdit && onEventClick(ev.id)}
                ...
                <td style={{ padding: '6px 12px' }}>
                  {adminEdit ? (
                    <EditableStatus ev={ev} eventId={ev.id} onSaved={onEventsChange} />
                  ) : (
                    <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', alignItems: 'center' }}>
                      <StatoLavoroBadge valore={ev.statoLavoro} />
                      <StatoTermineBadge valore={ev.statoTermine} />
                    </div>
                  )}
                </td>
                <td style={tdStyle}>
