...
1. New Tables
  - `presentation_versions`
    ...
    - `template_name` (text)
    - `status` (text) - bozza, generazione_richiesta, pronto, errore
    - `notes` (text)
...
CREATE POLICY "anon_select_presentation_versions" ON presentation_versions FOR SELECT
  TO anon, authenticated USING (true);

DROP POLICY IF EXISTS "anon_insert_presentation_versions" ON presentation_versions;
CREATE POLICY "anon_insert_presentation_versions" ON presentation_versions FOR INSERT
  TO anon, authenticated WITH CHECK (true);

DROP POLICY IF EXISTS "anon_update_presentation_versions" ON presentation_versions;
CREATE POLICY "anon_update_presentation_versions" ON presentation_versions FOR UPDATE
  TO anon, authenticated USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "anon_delete_presentation_versions" ON presentation_versions;
CREATE POLICY "anon_delete_presentation_versions" ON presentation_versions FOR DELETE
  TO anon, authenticated USING (true);
