/*
# Harden upsert_event_member with owner, last-manager and role consistency checks

1. Modified Functions
   - `upsert_event_member` — replaced with additional validations

2. New Validations
   - EVENT_OWNER_PROTECTED: only admin or the PM themselves may update the PM's membership
   - LAST_EVENT_MANAGER: cannot downgrade the last manager (applies to all callers)
   - INVALID_RESPONSABILE_PERMISSIONS: responsabile must have can_manage_members=true
   - INVALID_READONLY_PERMISSIONS: sola_lettura must have all 7 permissions false

3. Security
   - SECURITY DEFINER, search_path = public, pg_temp
   - EXECUTE revoked from PUBLIC and anon, granted to authenticated
   - All prior behavior preserved (auth, escalation, invited_by, updated_at)

4. Important Notes
   - No table, RLS policy, or data changes
   - event_members count must remain 23
*/
