/*
# Add per-site rate limiting to submit_event_registration

## Summary
Adds a sliding-window rate limiter (30 submissions per minute per registration
site) to the public registration RPC. This prevents abuse (email enumeration,
form spam) without affecting legitimate traffic during real event openings.

## 1. New Tables
- `registration_rate_limits`
  - `site_id` (uuid, NOT NULL) — references the registration site being rate-limited
  - `window_start` (timestamptz, NOT NULL) — minute-truncated timestamp for the sliding window
  - `count` (int, NOT NULL, DEFAULT 1) — number of submissions in this window
  - PRIMARY KEY `(site_id, window_start)`

## 2. Security
- RLS enabled on `registration_rate_limits`.
- No policies created — the table is intentionally inaccessible to `anon` and
  `authenticated` roles via the Data API. Only the SECURITY DEFINER function
  (which runs as the function owner) can read/write it.
- Explicit REVOKE of all privileges from PUBLIC, anon, and authenticated.

## 3. Modified Functions
- `submit_event_registration` — CREATE OR REPLACE with identical signature.
  - New rate-limit block inserted after the site lookup and before input
    trimming / validation.
  - Wrapped in BEGIN … EXCEPTION WHEN undefined_table so registration still
    works if the table is ever missing.
  - On each call: deletes stale rows (> 1 hour) for the current site, then
    checks/increments the counter for the current minute window.
  - If count >= 30, returns the existing generic error REGISTRATION_NOT_COMPLETED
    (no new error code, indistinguishable from other failures).
  - All other logic, error codes, and the function signature are unchanged.

## Important Notes
1. Threshold of 30/minute/site gives 3-6x headroom over a realistic burst of
   5-10 registrations/minute during a popular event opening.
2. The rate-limit check uses FOR UPDATE to prevent race conditions under
   concurrent load.
3. Cleanup is bounded: only rows for the current site_id older than 1 hour are
   deleted, keeping the operation cheap.
4. No IP-based limiting in this migration — site_id only.
*/
...
REVOKE ALL ON registration_rate_limits FROM PUBLIC;
REVOKE ALL ON registration_rate_limits FROM anon;
REVOKE ALL ON registration_rate_limits FROM authenticated;

-- ─── 2. Replace submit_event_registration with rate-limit block ──────────
...
BEGIN
  ...
  -- Wrapped so that if the table is ever missing, registration still works.
  BEGIN
    v_rl_window := date_trunc('minute', now());

    -- Prune stale windows for this site (bounded, cheap)
    DELETE FROM registration_rate_limits
    WHERE site_id = v_site.id
      AND window_start < now() - interval '1 hour';

    -- Read + lock the current window counter
    ...
    -- Upsert: increment existing or create new window row
  ...
  EXCEPTION
    WHEN undefined_table THEN
      NULL; -- table missing: skip rate limiting, allow registration
  END;
